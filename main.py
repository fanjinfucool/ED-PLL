...
from utils.utils_loss import edl_mse_loss, edl_digamma_loss
...
parser.add_argument('-model', help='model name', type=str, default='convnet', choices=['linear', 'mlp', 'convnet', 'resnet'], required=False)
...
parser.add_argument('-annealing_step', help='annealing_step', type=float, default=1)
...
def main():
    ...
    for epoch in range(0, args.ep):
        ...
        for i, (images, labels, trues, indexes) in enumerate(train_loader): 
            ...
            loss, new_label = criterion(output, labels, epoch, num_classes, args.annealing_step, device)
